...
def add_page_number(canvas,_,team):
    ...
    # Adjust x position for team text to align it to the right
    team_x = width+230 - team_text_width  # Right margin padding of 10

    # Draw the team text at the calculated position
    canvas.drawString(team_x, y, team)
        ...
        if selected_columns:
            ...
            if st.button("Generate PDF",disabled=button_disabled):
                ...
                doc = SimpleDocTemplate(buffer, pagesize=landscape(A3), leftMargin=0 * inch,
                                        rightMargin=0 * inch, topMargin=0 * inch, bottomMargin= 0.7* inch)
                ...
                col_name_to_find = "Others"
                col_name_to_find2 = "File Sharing and WinSCP (Murex, Payroll, ATM Journal, etc)"
                columns_list = st.session_state.filtered_df.columns.tolist()


                idx_of_others_list = [idx for idx, col in enumerate(columns_list) if col == col_name_to_find]


                if col_name_to_find2 in columns_list:
                    idx_of_o = columns_list.index(col_name_to_find2)
                else:
                    idx_of_o = -1
                ...
                for idx, col in enumerate(columns_list):
                    if idx in [0, 1, 2]:
                        ...
                        len_num = 120
                    elif idx == idx_of_o or idx in idx_of_others_list:
                        len_num = b_col_size
                    else:
                        len_num = col_size
                        print(idx)
                ...
                print(idx_of_others_list)
                for i in range(last_index_of_70 + 1, len(col_widths)):
                    if i != idx_of_o and i not in idx_of_others_list:
